...
function PerformanceView() {
  const waves = useWaves() as any;

  const stats = [
    {
      label: "LLM CALLS",
      value: waves?.budgets?.llm_calls_today ?? "0",
      sub: "Today's factory load",
      icon: List
    },
    {
      label: "EST. COST",
      value: `$${waves?.budgets?.estimated_cost_usd?.toFixed(2) ?? "0.00"}`,
      sub: "API usage cost",
      icon: DollarSign
    },
    {
      ...
      sub: waves?.wave2?.vram_loaded ? "VRAM Active" : "Cold",
      icon: Zap
    },
    {
      label: "THROUGHPUT",
