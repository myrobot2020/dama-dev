...
const STAGES = [
  {
    key: "discovery",
    label: "Discovery",
    verbs: ["discovery.published", "ingest.start", "ingest.done", "download.start", "download.done"],
    wave: 0 as const,
  ...
  },
  {
    ...
    label: "Wave 1 — CPU",
    verbs: [
      "wave1.download.done", "wave1.extract.done", "wave1.segment.done",
      "audio.download.completed", "transcript.completed",
      "identify.start", "identify.done", "align.start", "align.done"
    ],
    wave: 1 as const,
  ...
  },
  {
    ...
    verbs: [
      ...
      "mcq.start", "mcq.done", "translate.start", "translate.done",
      "judge.start", "judge.done", "dub.start", "dub.done"
    ],
    wave: 2 as const,
  ...
  },
  {
    key: "wave3",
    label: "Wave 3 — Weaver",
    verbs: ["wave3.match.done", "wave3.weave.done", "wave3.validate.done", "embed.start", "embed.done"],
    wave: 3 as const,
    arts: ["weave", "image_match", "image_candidates"]
  },
  { key: "seal", label: "Seal", verbs: ["seal.uploaded", "embed.done"], wave: 3 as const, arts: ["seal", "final_json"] },
...
function SuttaDetail() {
  ...
  const handleReplay = async () => {
    ...
    try {
      ...
      const res = await client.replay?.(job.sutta_id);
      if (!res?.ok) {
        alert("Replay failed: " + (res?.error || "Unknown error"));
      } else {
        alert(`Replay report written: ${res.report}`);
      }
    } catch (e) {
      console.error(e);
      alert("Replay failed");
      ...
      <div>
        ...
        <div className="mt-2 flex items-center gap-3">
          ...
          <button
            ...
            className="inline-flex items-center gap-1.5 rounded-md border border-border px-2 py-1 font-mono text-[10px] uppercase tracking-widest hover:bg-secondary disabled:opacity-50"
          >
